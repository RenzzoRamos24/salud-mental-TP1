...
async function cargar() {
  cargando.value = true;
  try {
    items.value = await api.adminListarTodosContenidos();
  } catch (e) {
...
async function guardar() {
  ...
  try {
    if (editando.value)
      await api.adminActualizarContenido(editando.value.id, form.value);
...
<template>
  <div class="page-shell-wide">
    ...
    <PageHeader
      title="Contenidos"
      subtitle="Lo que los estudiantes ven en la sección de lecturas."
      tone="mint"
    >
      <template #actions>
        <button @click="abrirNuevo" class="btn-primary btn-sm mt-3">
          + Nuevo contenido
        </button>
      </template>
    </PageHeader>
    ...
    <div v-else-if="!items.length" class="card p-10 text-center">
      <p class="text-ink-500">Todavía no hay nada. Crea el primero.</p>
    </div>
          ...
          <tbody class="divide-y divide-ink-100">
            ...
            >
              <td class="px-5 py-3">
                ...
                <p class="text-xs text-ink-500 line-clamp-1">
                  {{ c.descripcion }}
                </p>
              </td>
              <td class="px-5 py-3">
                <span class="chip-brand capitalize">{{ c.tipo }}</span>
              </td>
              <td class="px-5 py-3 capitalize">
                <span v-if="c.categoria" class="dsm5-tag">{{
                  c.categoria
                }}</span>
              ...
              <td class="px-5 py-3 text-right whitespace-nowrap">
                <button @click="abrirEditar(c)" class="btn-ghost btn-sm">
                  Editar
                </button>
                <button
                  @click="eliminar(c)"
                  class="btn-ghost btn-sm !text-risk-critico"
                >
                  Eliminar
                </button>
              </td>
      ...
      >
        ...
        >
          ...
          <div class="grid sm:grid-cols-2 gap-3">
            ...
            <div class="sm:col-span-2">
              <label
                class="flex items-center gap-2 cursor-pointer p-3 rounded-xl bg-green-50 border border-green-100"
              >
                <input
                  v-model="form.activo"
                  type="checkbox"
                ...
                />
                <span class="text-sm text-ink-800 font-medium"
                  >Visible para estudiantes</span
                >
              </label>
            </div>
          </div>
          ...
          <div class="flex gap-2 mt-5">
            <button @click="modalAbierto = false" class="btn-ghost flex-1">
              Cancelar
            </button>
            <button
              @click="guardar"
