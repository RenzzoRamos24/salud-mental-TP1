...
<template>
  <AuthShell
    title="Crear cuenta"
    subtitle="Únete al espacio de bienestar emocional de la UPC"
  >
    <form @submit.prevent="registrar" class="space-y-4">
      <!-- Selector de rol -->
      <div>
        <label class="label">¿Cómo te registras?</label>
        <div class="grid grid-cols-2 gap-3">
          <label
            :class="[
              'cursor-pointer flex flex-col items-center gap-1 p-4 rounded-2xl border-2 transition',
              role === 'estudiante'
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input v-model="nombre" type="text" :disabled="cargando" class="input" />
        </div>
        <div>
          <label class="label">Apellido</label>
          <input v-model="apellido" type="text" :disabled="cargando" class="input" />
        </div>
      </div>

      <div>
        <label class="label">Correo institucional</label>
        <input v-model="email" type="email" :disabled="cargando" placeholder="tucorreo@upc.edu.pe" class="input" />
      </div>

      <div>
        <label class="label">Contraseña</label>
        <input v-model="password" type="password" :disabled="cargando" minlength="8" placeholder="••••••••" class="input" />
        <p class="field-hint">Mínimo 8 caracteres</p>
      </div>

      <div>
        <label class="label">Confirmar contraseña</label>
        <input v-model="password2" type="password" :disabled="cargando" placeholder="••••••••" class="input" />
      </div>
    ...
    <template #footer>
      <p class="text-sm text-ink-500">
        ¿Ya tienes cuenta?
        <router-link to="/login" class="text-brand-600 hover:text-brand-700 font-semibold">Inicia sesión</router-link>
      </p>
    </template>
  </AuthShell>
</template>
