...
async function cargarEncuesta() {
  encuesta.value = await api.adminGetEncuesta();
}
async function guardarEncuesta() {
  encuestaGuardando.value = true;
  encuestaMensaje.value = "";
  try {
    await api.adminUpdateEncuesta(
      encuesta.value.preguntas,
      encuesta.value.frecuencia_dias,
    );
    encuestaMensaje.value = "Cambios guardados.";
  } catch (e) {
    encuestaMensaje.value = "Error: " + (e.response?.data?.detail || e.message);
  } finally {
    encuestaGuardando.value = false;
  }
}
...
<template>
  <div class="page-shell-wide">
    ...
    <PageHeader
      title="Configuración"
      subtitle="Encuestas, auditoría, respaldos y modelo."
      tone="sky2"
    />
    ...
    <section v-if="tab === 'encuesta'" class="card p-6 fade-in-up">
      ...
      <div v-if="encuesta" class="mt-4 space-y-5">
        <div>
          ...
          />
          <p class="field-hint">
            Periodo sugerido para que el estudiante repita la evaluación.
          </p>
        </div>

        <div>
          <h3
            class="text-sm font-semibold text-ink-700 mb-2 flex items-center gap-2"
          >
          ...
          <ol class="space-y-2 text-sm text-ink-700 mt-2">
            <li
              v-for="(p, i) in encuesta.preguntas"
              :key="i"
              class="card-pastel p-3 flex gap-2"
            >
              <span class="font-bold text-green-700 shrink-0"
                >{{ i + 1 }}.</span
              >
          ...
          <p class="field-hint mt-2">
            Las preguntas son fijas. Aquí solo cambias la frecuencia.
          </p>
        </div>

        <button
          @click="guardarEncuesta"
          :disabled="encuestaGuardando"
          class="btn-primary"
        >
          {{ encuestaGuardando ? "Guardando…" : "Guardar cambios" }}
        </button>
        <p v-if="encuestaMensaje" class="text-sm text-ink-600 mt-3">
          {{ encuestaMensaje }}
        </p>
      </div>
    </section>
    ...
    <section v-if="tab === 'bert'" class="space-y-6 fade-in-up">
      ...
      <div class="card p-6">
        <h2 class="section-title">Umbrales de detección</h2>
        <p class="section-subtitle mb-4">
          A partir de qué porcentaje se considera detectada una condición.
        </p>
