...
const nivelTone = computed(() => {
  const map = {
    'CRÍTICO': { bg: 'from-red-50 to-peach-50',     border: 'border-red-200',     text: 'text-risk-critico', emoji: '🚨' },
    'ALTO':    { bg: 'from-peach-50 to-amber-50',   border: 'border-peach-200',   text: 'text-peach-600',    emoji: '🟠' },
    'MEDIO':   { bg: 'from-amber-50 to-mint-50',    border: 'border-amber-200',   text: 'text-amber-700',    emoji: '🟡' },
    'BAJO':    { bg: 'from-mint-50 to-brand-50',    border: 'border-mint-200',    text: 'text-mint-600',     emoji: '🌿' },
  }
...
const conclusionPsicologo = computed(() => {
  ...
  if (conds.length === 0) {
    ...
    if (nivel === 'CRÍTICO') resumen += 'ATENCIÓN URGENTE: riesgo suicida activo requiere evaluación inmediata y plan de seguridad.'
    else if (nivel === 'ALTO') resumen += 'Se sugiere evaluación clínica estructurada (PHQ-9, GAD-7) y plan de intervención.'
    else if (nivel === 'MEDIO') resumen += 'Monitoreo recomendado; considerar psicoterapia breve y reevaluación en 2–4 semanas.'
  }
  return resumen
})
...
function construirGrafico() {
  ...
  chartInstance = new Chart(canvas.value, {
    type: 'bar',
    data: { labels, datasets: [{ label: 'Confianza (%)', data, backgroundColor, borderRadius: 8, borderWidth: 0 }] },
    options: {
      indexAxis: 'y',
      responsive: true,
        ...
        tooltip: {
          callbacks: {
            afterLabel: (ctx) => {
              ...
              const det = data[ctx.dataIndex] >= u ? '✅ Supera umbral' : '⚪ Bajo umbral'
              return `Umbral: ${u}%  ·  ${det}`
            },
          },
        },
      ...
      scales: {
        x: { beginAtZero: true, max: 100, ticks: { callback: v => v + '%' }, grid: { color: '#EDEAF0' } },
    ...
    <div class="space-y-6">
      ...
      <header class="fade-in-up">
        <div class="flex flex-wrap items-end justify-between gap-4 mb-2">
          <div>
            <p class="text-xs uppercase tracking-widest text-mint-500 font-bold">Informe clínico preliminar</p>
            <h1 class="hero-serif text-3xl sm:text-4xl mt-1">Perfil <span class="hero-mint">emocional</span></h1>
            <p class="text-ink-500 mt-2">
              <span class="font-semibold text-ink-700">{{ resultado.usuario }}</span>
              <span class="mx-2 text-ink-300">·</span>
              {{ fechaFormateada }}
            </p>
          </div>
          <div class="flex gap-2 no-print">
            <button @click="imprimir" class="btn-secondary">🖨️ Imprimir / PDF</button>
            <button @click="emit('reset')" class="btn-mint">Nueva evaluación</button>
          </div>
        </div>
      </header>
      ...
      <section class="grid md:grid-cols-3 gap-5 fade-in-up">
        <div
          class="md:col-span-2 rounded-2xl2 border-l-4 p-6 shadow-soft bg-gradient-to-br"
          :class="[nivelTone.bg, nivelTone.border]"
        >
          <p class="text-xs uppercase tracking-widest text-ink-500 font-bold">Nivel de riesgo global</p>
          <div class="flex items-center gap-3 mt-2">
            <span class="text-4xl">{{ nivelTone.emoji }}</span>
            <span class="text-4xl font-bold" :class="nivelTone.text">{{ nivelRiesgo }}</span>
          </div>
        ...
        <div class="card p-6">
          <p class="text-xs uppercase tracking-widest text-ink-500 font-bold">Resumen</p>
          <div class="mt-3 space-y-3">
            ...
            <div class="flex justify-between text-sm">
              ...
              <span class="font-bold text-ink-900">{{ resultado.respuestas_analizadas }}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-ink-500">Sesión</span>
              <span class="font-mono text-xs text-ink-400 truncate max-w-[120px]" :title="resultado.session_id">
      ...
      <section v-if="numDetectadas > 0" class="card p-6 fade-in-up">
        ...
        <div class="grid sm:grid-cols-2 gap-4 mt-4">
          <div
            ...
            :key="clave"
            class="rounded-2xl border border-ink-100 p-4 hover:shadow-soft transition"
          >
            ...
            <div class="h-2 bg-ink-100 rounded-full overflow-hidden">
              <div
                ...
                :style="{ width: cond.confianza + '%', backgroundColor: COLORES[clave] || '#9A93A4' }"
              ></div>
            </div>
          </div>
        </div>
      ...
      <section v-if="numDetectadas > 0" class="card p-6 fade-in-up">
        ...
        <p class="section-subtitle mb-4">Pasos concretos que puedes empezar a aplicar hoy.</p>
        <div class="space-y-4 mt-4">
          <div
            ...
            :class="clave === 'riesgo_suicida' ? 'bg-red-50 border-red-200' : 'bg-brand-50 border-brand-100'"
          >
            <div class="flex items-center justify-between mb-3">
            ...
            <ol class="space-y-2 text-sm text-ink-700">
              <li v-for="(paso, i) in RECOMENDACIONES[clave]?.pasos || []" :key="i" class="flex gap-3">
                <span class="shrink-0 w-6 h-6 rounded-full bg-white border border-brand-200 text-brand-700 font-bold text-xs flex items-center justify-center">{{ i + 1 }}</span>
                <span>{{ paso }}</span>
              </li>
            </ol>
          </div>
      ...
      <section v-else class="card-mint p-6 fade-in-up">
        <div class="flex items-start gap-4">
          ...
          <div>
            <h2 class="text-xl font-bold text-mint-600">¡Buen estado emocional!</h2>
            <p class="text-ink-700 text-sm mt-2 leading-relaxed">
              No se detectaron condiciones clínicas significativas. Mantén tus hábitos protectores:
              sueño regular, actividad física, contacto con tu red de apoyo y momentos de descanso.
              Te recomendamos hacer esta evaluación cada 1–2 semanas.
            </p>
          </div>
        </div>
      </section>
      ...
      <section class="card p-6 fade-in-up">
        ...
        <pre class="whitespace-pre-wrap text-sm text-ink-700 font-sans leading-relaxed bg-cream-50 p-5 rounded-2xl border border-ink-100 mt-3">{{ r.explicacion }}</pre>
      </section>
      ...
      <section class="card p-6 fade-in-up">
        ...
        <div class="grid md:grid-cols-2 gap-5 text-sm mt-4">
          <div class="space-y-3">
            <div>
              <p class="text-xs uppercase tracking-wider font-semibold text-ink-400">Modelo</p>
              <p class="font-mono text-xs text-ink-700 break-all">{{ r.modelo }}</p>
            </div>
            <div>
              <p class="text-xs uppercase tracking-wider font-semibold text-ink-400">Técnica</p>
              <p class="text-ink-700">Zero-shot multi-label classification (BERT + XNLI)</p>
            </div>
            <div>
              <p class="text-xs uppercase tracking-wider font-semibold text-ink-400">Idioma</p>
              <p class="text-ink-700">Español (sin traducción intermedia)</p>
            </div>
          </div>
          <div class="space-y-3">
            <div>
              <p class="text-xs uppercase tracking-wider font-semibold text-ink-400">Escalas de referencia</p>
            ...
            <div>
              <p class="text-xs uppercase tracking-wider font-semibold text-ink-400">Umbrales de detección</p>
              <p class="text-ink-700 text-xs leading-relaxed">
                Depresión / Ansiedad / Estrés / Soledad: <strong>55%</strong> · TDAH: <strong>50%</strong> ·
                <span class="text-risk-critico font-semibold">Riesgo suicida: 40%</span> (alta sensibilidad)
              </p>
            </div>
          </div>
        </div>
      ...
      <section class="card-hero p-6 fade-in-up bg-gradient-to-br from-mint-50 to-peach-50">
        <h2 class="section-title text-brand-800">👨‍⚕️ Conclusión para revisión profesional</h2>
        <p class="text-ink-800 leading-relaxed mt-3">{{ conclusionPsicologo }}</p>
        <div class="mt-4 p-4 bg-white rounded-2xl border border-brand-100 text-xs text-ink-600">
      ...
      <footer class="text-center text-xs text-ink-400 pb-2">
        Sistema desarrollado como prototipo de tesis ·
